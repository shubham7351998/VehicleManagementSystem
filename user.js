...
console.log("backshow called");
 var posts=[current_user.user_name];
console.log(current_user);
                var sqls="select Tiket.j_date,Tiket.route_id,Tiket.vehicle_id,Tiket.seats,Payment.amount,V_Routes.source,V_Routes.destination from Tiket inner join Payment on Payment.tiket_no=Tiket.tiket_no  inner join V_Routes on V_Routes.route_id=Tiket.route_id where Tiket.user_name=?";
...
app.get("/confirm",(req,res)=>{
    ...
    var post={
     ...
     var sql2="INSERT INTO Tiket SET ?";
...
app.get("/booktikets",function(req,res){
 ...
 //logout
app.get("/logoutpaige",function(req,res)
{
    ...
    if(passnotmatch=="NO" && usermatch=="NO")
{
    ...
    var post={user_name:u_name,city:req.body.city,password:pass,name:req.body.name,dob:req.body.dob};
    var sql2="INSERT INTO U_Table SET ?"
    db.query(sql2,post,(err,results)=>{
    if(err){throw err;}
    else{
        console.log("Values Inserted ");
        var post2=[u_name];
        var sql3="SELECT * FROM U_Table WHERE user_name=?";
        console.log(u_name);
...
app.post("/profile",function(req,res) {nft
   ...
   db.query(sql,(err,results)=>{
      ...
      for(var i=0;i<results.length;i++)
      {
          ...
          /*else {
            res.render("worng.ejs",{file:{xyz:"User Name nft!"}});
...
app.post("/booking_data",function(req,res){
   ...
   console.log(req.body);
    ...
    var sql="SELECT V_Routes.source, V_Routes.route_id,V_Routes.destination,join_route.booleanstring,join_route.vehicle_id,join_route.fare FROM V_Routes INNER JOIN join_route ON V_Routes.route_id=join_route.route_id where V_Routes.source=? and V_Routes.destination=? and join_route.booleanstring like ?"; 
...
console.log(BookingInformation);
    ...
    var sql="SELECT COUNT('tiket_no') AS nft FROM Tiket";
    db.query(sql,(err,results)=>{
    if(err) {
      ...
      db.query(sql2,post,(err,results)=>{
        ...
        var post2={trans_id:tansid,amount:paymentamountss,about:"XYX",tiket_no:t};
        var sql3="INSERT INTO Payment SET ?";
        db.query(sql3,post2,(err,resu)=>{
            if(err){res.send("Error");}
            else
            {
                res.render("ticketConfirm.ejs",{file:current_user,data:BookingInformation,d:str1});
            }
        });
...
app.post("/employee/loginSubmit",function(req,res) {
    console.log(req.body);
...
//employeepass
app.get("/employee/changepassword",function(req,res)
{
...
if(current_employee.password==cp)
{
  if(pn==pnr)
  {
      var post=[pn,current_employee.emp_id];
      var sql="UPDATE Employee SET password=? WHERE emp_id=?";
    db.query(sql,post,(err,results)=>{
...
app.post("/employee/updatevehicle/:id",function(req,res)
{
    ...
    if(updateoption==1)
    {
     ...
     var sql="UPDATE Vehicles SET fuel=? WHERE vehicle_id=?";
    ...
    else if(updateoption==2)
    {
     ...
     var sql="UPDATE Vehicles SET lastservice=? WHERE vehicle_id=?";
     db.query(sql,post,function(err,results){
      if(err) throw err;
      console.log("inserted");
     });
    ...
    else if(updateoption==3)
    {
        ...
        var sql="UPDATE Vehicles SET garage_no=? WHERE vehicle_id=?";
        db.query(sql,post,function(err,results){
         if(err) throw err;
         console.log("inserted");
        });
